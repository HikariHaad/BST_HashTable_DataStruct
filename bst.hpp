...
class BST
{
  ...
  public:
    BST();									//constructor
    ~BST();	//destructor
    void DestroyRecursive(Node<T1,T2> *ptr);	//recursive destructor)
    ...
    Node<T1,T2>* remove(Node<T1,T2> *ptr,T1 key, T2 value);			//Remove a node with key from the tree/subtree with root
};
//=============================================================================
